...
import {
  decodeMetadata, getSafecoinMetadataAddress, Metadata
} from "../utils/metaplex";
import { getMultipleAccountsRPC } from "../utils/solana";

export const getSafecoinMetaplexData = async (mintAddresses: string[]) => {
  const promises = [];
  for (const address of mintAddresses) {
    promises.push(getSafecoinMetadataAddress(address));
  }
...
const useSafecoinMetaplexData = (
  ...
  useLayoutEffect(() => {
    let cancelled = false;
    setIsLoading(true);
    getSafecoinMetaplexData(addresses).then(
      (results) => {
        if (!cancelled) {
...
export default useSafecoinMetaplexData;
