...
import { getSignedVAAHashSafecoin, getSignedVAAHashSolana } from "../bridge";
import { importCoreWasm as importCoreWasmSafecoin } from "../safecoin/wasm";
import { importCoreWasm as importCoreWasmSolana } from "../solana/wasm";
...
export async function getIsTransferCompletedEth(
  ...
  // TODO(Victor): Check the case below
  const signedVAAHash = await getSignedVAAHashSolana(signedVAA);
  // const signedVAAHash = await getSignedVAAHashSafecoin(signedVAA);
...
export async function getIsTransferCompletedSafecoin(
  tokenBridgeAddress: string,
  signedVAA: Uint8Array,
  connection: Connection
) {
  const { claim_address } = await importCoreWasmSafecoin();
...
export async function getIsTransferCompletedSolana(
  ...
  const { claim_address } = await importCoreWasmSolana();
  const claimAddress = await claim_address(tokenBridgeAddress, signedVAA);
  const claimInfo = await connection.getAccountInfo(
    new PublicKey(claimAddress),
    "confirmed"
