...
fn run_integration_tests() {
    ...
    // Tests are currently unhygienic as It's difficult to wrap `safecoin-test-validator` within the
...
fn test_invalid_emitter(context: &mut Context) {
    ...
    assert!(common::execute(
        ...
        safecoin_sdk::commitment_config::CommitmentConfig::processed(),
    )
    .is_err());
}
...
fn test_guardian_set_change(context: &mut Context) {
    // Initialize a wormhole bridge on Safecoin to test with.
    let (ref payer, ref client, ref program) = common::setup();
