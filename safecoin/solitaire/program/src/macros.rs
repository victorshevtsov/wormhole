...
/// A wrapper around Safecoin's `msg!` macro that is a no-op by default, allows for adding traces
...
macro_rules! trace_impl {
    ( $($arg:tt)* ) => { safecoin_program::msg!( $($arg)* ) };
}
...
macro_rules! solitaire {
    { $($row:ident($kind:ty) => $fn:ident),+ $(,)* } => {
        pub mod instruction {
            ...
            pub fn solitaire(p: &Pubkey, a: &[AccountInfo], d: &[u8]) -> ProgramResult {
                ...
                if let Err(err) = dispatch(p, a, d) {
                    safecoin_program::msg!("Error: {:?}", err);
                    return Err(err.into());
                }
                Ok(())
            }
        ...
        safecoin_program::entrypoint!(solitaire);
    }
}
    ...
    ($name:ident, $embed:ty, $owner:expr) => {
        ...
        impl BorshDeserialize for $name {
            fn deserialize(buf: &mut &[u8]) -> std::io::Result<Self> {
                let acc = $name(
                    safecoin_program::program_pack::Pack::unpack(buf)
        ...
        impl BorshSerialize for $name {
            fn serialize<W: std::io::Write>(&self, writer: &mut W) -> std::io::Result<()> {
                let mut data = [0u8; <$embed as safecoin_program::program_pack::Pack>::LEN];
                safecoin_program::program_pack::Pack::pack_into_slice(&self.0, &mut data);
                writer.write(&data)?;
