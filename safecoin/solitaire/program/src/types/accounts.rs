//! Accounts.
//!
//! Safecoin provides a single primitive `AccountInfo` that represents an account on Safecoin. It
...
use safecoin_program::{
    ...
    pubkey::Pubkey,
    system_instruction,
    sysvar::Sysvar as SafecoinSysvar,
};
use std::ops::{
    Deref,
    DerefMut,
...
///
/// Safecoin's Rust version is JUST old enough that it cannot use constant variables in its default
...
pub struct Sysvar<'b, Var: SafecoinSysvar>(pub AccountInfo<'b>, pub Var);

impl<'b, Var: SafecoinSysvar> Deref for Sysvar<'b, Var> {
    type Target = Var;
    fn deref(&self) -> &Self::Target {
        &self.1
    }
