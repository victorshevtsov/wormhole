        ...
        Data::Struct(DataStruct {
            ...
            #[cfg(feature = "client")]
                impl #impl_generics  solitaire_client::ToInstruction for #client_struct_name {
                    fn to_ix(
                self,
                program_id: safecoin_program::pubkey::Pubkey,
                ix_data: &[u8]) -> std::result::Result<
                (solitaire_client::Instruction, Vec<solitaire_client::Keypair>),
                            solitaire::ErrBox
                > {
            use safecoin_program::{pubkey::Pubkey, instruction::Instruction};
            ...
            // Add dependencies
            deps.dedup();
            let mut dep_ams = deps.iter().map(|v| safecoin_program::instruction::AccountMeta::new_readonly(*v, false)).collect();
            account_metas.append(&mut dep_ams);

            Ok((safecoin_program::instruction::Instruction::new_with_bytes(program_id,
                                             ix_data,
