    ...
    T: DeserializePayload,
{
    ...
    // Fail if the emitter is not the known governance key, or the emitting chain is not Safecoin.
    if expected_emitter != current_emitter || vaa.message.meta().emitter_chain != CHAIN_ID_SAFECOIN {
...
pub fn register_chain(
    ...
    if accs.vaa.chain == CHAIN_ID_SAFECOIN {
