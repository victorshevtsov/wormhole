...
pub fn transfer_native_ix(
    ...
    panic::set_hook(Box::new(console_error_panic_hook::hook));
    console::log_1(&"*** DEBUG *** transfer_native_ix 01".into());

    let program_id = Pubkey::from_str(program_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix 02".into());

    let bridge_id = Pubkey::from_str(bridge_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix 03".into());

    let payer = Pubkey::from_str(payer.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix 04".into());

    let message = Pubkey::from_str(message.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix 05".into());

    let from = Pubkey::from_str(from.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix 06".into());

    let mint = Pubkey::from_str(mint.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix 07".into());

    let mut target_addr = [0u8; 32];
    target_addr.copy_from_slice(target_address.as_slice());
    console::log_1(&"*** DEBUG *** transfer_native_ix 08".into());
    ...
    .unwrap();
    console::log_1(&"*** DEBUG *** transfer_native_ix Ok".into());
...
pub fn transfer_wrapped_ix(
    ...
    panic::set_hook(Box::new(console_error_panic_hook::hook));
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 01".into());

    let program_id = Pubkey::from_str(program_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 02".into());

    let bridge_id = Pubkey::from_str(bridge_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 03".into());

    let payer = Pubkey::from_str(payer.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 04".into());

    let message = Pubkey::from_str(message.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 05".into());

    let from = Pubkey::from_str(from.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 06".into());

    let from_owner = Pubkey::from_str(from_owner.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 07".into());

    let mut target_addr = [0u8; 32];
    target_addr.copy_from_slice(target_address.as_slice());
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 08".into());

    let mut token_addr = [0u8; 32];
    token_addr.copy_from_slice(token_address.as_slice());
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix 09".into());
    ...
    .unwrap();
    console::log_1(&"*** DEBUG *** transfer_wrapped_ix Ok".into());
...
pub fn complete_transfer_native_ix(
    ...
    panic::set_hook(Box::new(console_error_panic_hook::hook));
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 01".into());

    let program_id = Pubkey::from_str(program_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 02".into());

    let bridge_id = Pubkey::from_str(bridge_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 03".into());

    let payer = Pubkey::from_str(payer.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 04".into());

    let vaa = VAA::deserialize(vaa.as_slice()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 05".into());

    let payload = PayloadTransfer::deserialize(&mut vaa.payload.as_slice()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 06".into());
    ...
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 07".into());
    ...
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix 08".into());
    ...
    .unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_native_ix Ok".into());
...
pub fn complete_transfer_wrapped_ix(
    ...
    panic::set_hook(Box::new(console_error_panic_hook::hook));
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 01".into());

    let program_id = Pubkey::from_str(program_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 02".into());

    let bridge_id = Pubkey::from_str(bridge_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 03".into());

    let payer = Pubkey::from_str(payer.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 04".into());

    let vaa = VAA::deserialize(vaa.as_slice()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 05".into());

    let payload = PayloadTransfer::deserialize(&mut vaa.payload.as_slice()).unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 06".into());
    ...
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 07".into());
    ...
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix 08".into());
    ...
    .unwrap();
    console::log_1(&"*** DEBUG *** complete_transfer_wrapped_ix Ok".into());
...
pub fn create_wrapped_ix(
    ...
    panic::set_hook(Box::new(console_error_panic_hook::hook));
    console::log_1(&"*** DEBUG *** create_wrapped_ix 01".into());

    let program_id = Pubkey::from_str(program_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** create_wrapped_ix 02".into());

    let bridge_id = Pubkey::from_str(bridge_id.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** create_wrapped_ix 03".into());

    let payer = Pubkey::from_str(payer.as_str()).unwrap();
    console::log_1(&"*** DEBUG *** create_wrapped_ix 04".into());

    let vaa = VAA::deserialize(vaa.as_slice()).unwrap();
    console::log_1(&"*** DEBUG *** create_wrapped_ix 05".into());

    let payload = PayloadAssetMeta::deserialize(&mut vaa.payload.as_slice()).unwrap();
    console::log_1(&"*** DEBUG *** create_wrapped_ix 06".into());
    ...
    console::log_1(&"*** DEBUG *** create_wrapped_ix 07".into());
    ...
    console::log_1(&"*** DEBUG *** create_wrapped_ix 08".into());
    ...
    .unwrap();
    console::log_1(&"*** DEBUG *** create_wrapped_ix Ok".into());
