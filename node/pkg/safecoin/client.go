...
import (
	...
	"github.com/gagliardetto/solana-go"             // TODO(Victor): Consider using right lib for Safecoin instead of Solana
	"github.com/gagliardetto/solana-go/rpc"         // TODO(Victor): Consider using right lib for Safecoin instead of Solana
	"github.com/gagliardetto/solana-go/rpc/jsonrpc" // TODO(Victor): Consider using right lib for Safecoin instead of Solana
...
var (
	safecoinConnectionErrors = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_safecoin_connection_errors_total",
			Help: "Total number of Safecoin connection errors",
		}, []string{"commitment", "reason"})
	safecoinAccountSkips = promauto.NewCounterVec(
		prometheus.CounterOpts{
			Name: "wormhole_safecoin_account_updates_skipped_total",
			Help: "Total number of account updates skipped due to invalid data",
		}, []string{"reason"})
	safecoinMessagesConfirmed = promauto.NewCounter(
		prometheus.CounterOpts{
			Name: "wormhole_safecoin_observations_confirmed_total",
			Help: "Total number of verified Safecoin observations found",
		})
	currentSafecoinHeight = promauto.NewGaugeVec(
		prometheus.GaugeOpts{
			Name: "wormhole_safecoin_current_height",
			Help: "Current Safecoin slot height",
		}, []string{"commitment"})
	queryLatency = promauto.NewHistogramVec(
		prometheus.HistogramOpts{
			Name: "wormhole_safecoin_query_latency",
			Help: "Latency histogram for Safecoin RPC calls",
		}, []string{"operation", "commitment"})
)

const rpcTimeout = time.Second * 5

// Maximum retries for Safecoin fetching
const maxRetries = 5
...
func NewSafecoinWatcher(
	wsUrl, rpcUrl string,
	contractAddress solana.PublicKey,
	messageEvents chan *common.MessagePublication,
	commitment rpc.CommitmentType) *SafecoinWatcher {
	return &SafecoinWatcher{
...
func (s *SafecoinWatcher) Run(ctx context.Context) error {
	// Initialize gossip metrics (we want to broadcast the address even if we're not yet syncing)
	contractAddr := base58.Encode(s.contract[:])
	p2p.DefaultRegistry.SetNetworkStats(vaa.ChainIDSafecoin, &gossipv1.Heartbeat_Network{
	...
	go func() {
		...
		for {
			...
			case <-timer.C:
				...
				if err != nil {
					p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSafecoin, 1)
					safecoinConnectionErrors.WithLabelValues(string(s.commitment), "get_slot_error").Inc()
					errC <- err
					return
				}
				if lastSlot == 0 {
				...
				currentSafecoinHeight.WithLabelValues(string(s.commitment)).Set(float64(slot))
				readiness.SetReady(common.ReadinessSafecoinSyncing)
				p2p.DefaultRegistry.SetNetworkStats(vaa.ChainIDSafecoin, &gossipv1.Heartbeat_Network{
					Height:          int64(slot),
					ContractAddress: contractAddr,
				})
				logger.Info("fetched current Safecoin height",
...
func (s *SafecoinWatcher) retryFetchBlock(ctx context.Context, logger *zap.Logger, slot uint64, retry uint) {
...
func (s *SafecoinWatcher) fetchBlock(ctx context.Context, logger *zap.Logger, slot uint64, emptyRetry uint) (ok bool) {
	...
	if err != nil {
		...
		if errors.As(err, &rpcErr) && (rpcErr.Code == -32007 /* SLOT_SKIPPED */ || rpcErr.Code == -32004 /* BLOCK_NOT_AVAILABLE */) {
			...
			// Schedule a single retry just in case the Safecoin node was confused about the block being missing.
			...
			logger.Error("failed to request block", zap.Error(err), zap.Uint64("slot", slot),
				zap.String("commitment", string(s.commitment)))
			p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSafecoin, 1)
			safecoinConnectionErrors.WithLabelValues(string(s.commitment), "get_confirmed_block_error").Inc()
		}
		return false
	}
...
OUTER:
	for _, tx := range out.Transactions {
		...
		if err != nil {
			p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSafecoin, 1)
			safecoinConnectionErrors.WithLabelValues(string(s.commitment), "get_confirmed_transaction_error").Inc()
...
func (s *SafecoinWatcher) processInstruction(ctx context.Context, logger *zap.Logger, slot uint64, inst solana.CompiledInstruction, programIndex uint16, tx rpc.TransactionWithMeta, signature solana.Signature, idx int) (bool, error) {
...
func (s *SafecoinWatcher) retryFetchMessageAccount(ctx context.Context, logger *zap.Logger, acc solana.PublicKey, slot uint64, retry uint) {
...
func (s *SafecoinWatcher) fetchMessageAccount(ctx context.Context, logger *zap.Logger, acc solana.PublicKey, slot uint64) (retryable bool) {
	// Fetching account
	rCtx, cancel := context.WithTimeout(ctx, rpcTimeout)
	defer cancel()
	start := time.Now()
	...
	if err != nil {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSafecoin, 1)
		safecoinConnectionErrors.WithLabelValues(string(s.commitment), "get_account_info_error").Inc()
	...
	if !info.Value.Owner.Equals(s.contract) {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSafecoin, 1)
		safecoinConnectionErrors.WithLabelValues(string(s.commitment), "account_owner_mismatch").Inc()
	...
	if string(data[:3]) != "msg" {
		p2p.DefaultRegistry.AddErrorCount(vaa.ChainIDSafecoin, 1)
		safecoinConnectionErrors.WithLabelValues(string(s.commitment), "bad_account_data").Inc()
...
func (s *SafecoinWatcher) processMessageAccount(logger *zap.Logger, data []byte, acc solana.PublicKey) {
	proposal, err := ParseTransferOutProposal(data)
	if err != nil {
		safecoinAccountSkips.WithLabelValues("parse_transfer_out").Inc()
		logger.Error(
